...
function SRTF(at, bt) {
    ...
    console.log(ghantt);console.log(id);
...
function priorityScheduling(processes) {
    ...
    adddata.innerHTML=`Avg. W.T : ${(averageWaitingTime).toFixed(2)} <br/> Avg. T.A.T : ${(averageTurnAroundTime).toFixed(2)}`;
  }
...
function Process(id, arrivalTime, burstTime, priority) {
  ...
  function priorityPreemptiveScheduling(processes) {
    ...
    processes.forEach(process => {
      ...
      tot_tat+=processes.turnAroundTime;
      process.waitingTime = process.turnaroundTime - process.burstTime;
      tot_wt+=processes.waitingTime;
    });
